...
# load('./bac_model_diversity.RData')
...
#2------------参与建模的OTU多样性分析
ps_add_out = readRDS("./ps_final_model.rds")
ps_add_out 
path = getwd()
dir.create("./model_diversity")
path = paste(path,"/model_diversity/",sep = "")
...
```{R}
ps_add_out = readRDS("./ps_model_40.rds")
path = getwd()

dir.create("./RF_load_diversity")
path = paste(path,"/RF_load_diversity/",sep = "")
...
mapping = as.data.frame(sample_data(i))
cs = table(mapping$SampleType)
head(Taxonomies)

Repeated_quantityDbulk = cs [1]
Repeated_quantityHbulk = cs [2]



abundance = rep(0,nrow(Taxonomies))
for(ii in 1:nrow(Taxonomies)){
  if(Taxonomies[ii,4] == "D_bulk" ){
    abundance[ii] = Taxonomies$Abundance[ii] / Repeated_quantityDbulk
  }else if(Taxonomies[ii,4] == "H_bulk" ){
    abundance[ii] = Taxonomies$Abundance[ii] / Repeated_quantityHbulk
  }
}
...
p1 =p1+theme_bw()+
  ...
  theme(
    ...
    axis.text.y = element_text(colour = "black",size = 24,family="Times New Roman"),
...
library("Cairo")
ggsave(FileName1, p1, width = 15, height =6, device = cairo_pdf,  family = "Times New Roman" )
...
FileName2 
ggsave(FileName2, p2, width = 5.5, height =6, device = cairo_pdf, family = "Times New Roman" )
...
p3 = plot_alpha(ps11, x="SampleType",color="SampleType", measures=c( "Shannon","Chao1"),index = c("Pielou"))+
...
p3 = p3+mythemealpha+
  scale_x_discrete(limits = c(axis_order),labels = xlabel)+
  scale_color_manual(values = mi, guide = guide_legend(title = NULL),labels = xlabel)+
  scale_fill_manual(values = mi, guide = guide_legend(title = NULL),labels = xlabel)
p3   


library("Cairo")

plotname = paste(path,"/a1_alpha_diversity.pdf",sep = "")
ggsave(plotname , p3, width = 10, height = 6,limitsize = FALSE,device = cairo_pdf,family = "Times New Roman")
...
ado = adonis(otu_table~ sub_design$SampleType,permutations = 999,method="bray")
...
title
mi = c("#E41A1C","#377EB8" )
# points$SampleType = factor(points$SampleType,order = FALSE,levels = c("H_bulk","D_bulk"))
# mi=c("#1B9E77" ,"#D95F02", "#7570B3","#E7298A","#E6AB02", "#B3DE69")
p4 <-ggplot(points, aes(x=x, y=y, fill = SampleType)) +
  ...
  scale_colour_manual(values = mi,guide = guide_legend(title = NULL),labels = xlabel)+
  scale_fill_manual(values = mi,guide = guide_legend(title = NULL),labels = xlabel)+
...
ven2 = select(ven2,Health, everything())  
...
if (length(names(ven3)) == 2) {
  ...
  pdf(file=filename3,width = 8, height = 8)
...
```{R}
save.image(file='./bac_model_diversity.RData')
